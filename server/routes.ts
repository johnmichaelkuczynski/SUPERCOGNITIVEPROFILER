...
Return only the new content without any additional comments, explanations, or headers.`;
  ...
  app.post('/api/expand-chunks', async (req: Request, res: Response) => {
    try {
      const { 
        originalContent, 
        instructions, 
      ...
      if (!originalContent || !instructions || !chunks || !Array.isArray(chunks)) {
        return res.status(400).json({ error: 'Original content, instructions, and chunks array are required' });
      }

      const expandedChunks: string[] = [];

      // Process each chunk individually for intelligent subdivision
      for (let i = 0; i < chunks.length; i++) {
        const currentChunk = chunks[i];
        
        const prompt = `You are tasked with intelligently subdividing a single content chunk into multiple detailed sub-chunks.
...
SUBDIVISION INSTRUCTIONS: ${instructions}

Your task is to take this single chunk and break it down into approximately ${expansionFactor} detailed sub-chunks that:
1. Follow the specific subdivision instructions provided
2. Maintain the core content and logical structure of the original chunk
3. Create meaningful subdivisions that each contain substantial content
4. Add depth and detail while preserving the original meaning
5. Each sub-chunk should be complete and standalone

For example, if the original chunk contains a logical law like "Excluded Middle" with its definition and examples, you should subdivide it into separate chunks covering:
- The traditional logic interpretation
- The AI logic interpretation  
- How they differ
- Practical implications
- Examples and applications
- Etc.

Return only the subdivided chunks, separated by "---SUBCHUNK---" markers. Do not include any explanations or headers.`;
        ...
        const subChunks = result.split('---SUBCHUNK---')
      ...
      res.json({ 
        ...
        expandedChunkCount: expandedChunks.length,
        expansionRatio: Math.round((expandedChunks.length / chunks.length) * 10) / 10
      });

    } catch (error) {
      console.error('Chunk expansion error:', error);
      res.status(500).json({ error: 'Failed to expand chunks' });
    }
  });
