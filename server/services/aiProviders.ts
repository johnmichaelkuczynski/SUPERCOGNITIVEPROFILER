...
function expandPresets(selected: string[] = []): string[] {
  ...
  const add = (name: string) => {
    ...
    if (txt.includes(";") && !txt.includes("…")) {
      txt.split(";").map(s => s.trim()).forEach(alias => { 
        if (PRESET_TEXT[alias] && !seen.has(alias)) { 
          seen.add(alias); 
          out.push(alias); 
        }
      });
